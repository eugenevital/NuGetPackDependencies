...
# Namespace.
$nmm = New-Object -TypeName System.Xml.XmlNamespaceManager -ArgumentList $xmlNuspec.NameTable
$nmm.AddNamespace('x', 'http://schemas.microsoft.com/packaging/2010/07/nuspec.xsd')


# Remove all dependencies elements if present.
$tags =$xmlNuspec.package.metadata.SelectSingleNode("/x:package/x:metadata/x:dependencies", $nmm)
...
$metaDataElement = $xmlNuspec.SelectSingleNode('/x:package/x:metadata', $nmm)  
#$metaDataElement = $xmlNuspec.package.metadata

# Create new dependecies element
$newDependenciesChild = $xmlNuspec.CreateElement("dependencies", $metaDataElement.ParentNode.NamespaceURI) 
